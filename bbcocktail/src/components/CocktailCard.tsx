  ...
  return (
    <div className="h-full rounded-lg border border-gray-200 shadow-md overflow-hidden bg-white">
      <div className="relative">
        <img src={getImageUrl()} alt={cocktail.name || 'Cocktail'} className="w-full h-48 object-cover" />
        {cocktail.abv && (
      ...
      <div className="p-4">
        ...
        <div className="flex flex-wrap mb-2">
          {cocktail.method && <span className="mr-2 text-gray-600 text-sm">{cocktail.method}</span>}
        ...
        {/* Instructions */}
        {cocktail.instructions && <p className="text-gray-700 mb-2">{cocktail.instructions}</p>}

        {/* Glass and garnish info */}
        <p className="text-gray-500 text-sm">Serve in: {cocktail.glass}</p>
        {cocktail.garnish && <p className="text-gray-500 text-sm">Garnish: {cocktail.garnish}</p>}
      </div>
    </div>
  )
}
